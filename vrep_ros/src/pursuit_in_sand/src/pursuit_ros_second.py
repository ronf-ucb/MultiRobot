...
help_request_radius = float(abs(BaseFreq)) / 2
...
class Pursuit:
    def __init__(self, leftName, rightname):
        ...
        self.sub_stop = rospy.Subscriber("/cockroachFlag", Vector3, self.get_stop_sig, queue_size=1)
    ...
    def getPos(self, msg):
        ...
        print ( "true : ", (self.path.x - self.last_pose.x) ** 2 + (self.path.y - self.last_pose.y) ** 2)
        # print ( "expect : ", help_request_radius)
        if not self.help_request :
            if (self.path.x - self.last_pose.x) ** 2 + (self.path.y - self.last_pose.y) ** 2 < help_request_radius:    
                self.help_request = True

            
        if self.help_request:
            self.controller()
