...
class Decompose(object):
    ...
    def fitLSTM(self):
        ...
        class Network(nn.Module):
            def __init__(self, state, hidden, lr, horizon):
                super(Network, self).__init__()
                self.lstm = nn.LSTM(state, hidden_size = hidden)
                self.fc = nn.Linear(hidden, state)
        ...
        decomposer = Network(self.state_n, 32, 1e-4, self.horizon)
        ...
        # errors = .5*torch.sum(torch.abs(F.normalize(res, dim=1) - F.normalize(true_labels, dim=1)), dim=1).squeeze().detach().numpy()# torch.sum(abs(res - labels), dim=1).squeeze().detach().numpy()
