...
class BoxSlopeTask(Task):
    def __init__(self):
        ...
        self.goal_height = 3  
        self.x_contact = -.3
        self.contact = [-.25 + i*(.5)/(self.num_agents - 1) for i in range(self.num_agents)]
    ...
    def getPrimitive(self, angle, a, i): 
        ...
        if a == "STRAIGHT_BOX":
            action = np.array([self.forward_gain - damp*box_ori * 10 + np.sin(phi) * 5, self.forward_gain - damp*box_ori * 10 - np.sin(phi) * 5])
        if a == "HOME_CONTACT":
    ...
    def feature_joint_2_feature(self, feature):
        ...
        first_feature = np.hstack((feature[:5], feature[6:9], feature[18: 19]))
        second_feature = np.hstack((feature[9:14], feature[15:19]))
    ...
    def receiveState(self, msg):     
        ...
        if self.mode == 'GET_STATE_DATA':
            self.curr_rollout.extend(features)
        a = (self.sendAction(s))
        rest = 0
        for i in range(self.num_agents):
            loc = 'robot' + str(i)
    ...
    def postTraining(self):
        if self.mode == 'GET_STATE_DATA':
            self.data_to_txt(path = '/home/austinnguyen517/Documents/Research/BML/MultiRobot/AN_Bridging/' + 'SLOPE_PUSH' + '_state_data.txt')
            sys.exit(0)
        else:
