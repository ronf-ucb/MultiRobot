...
class Kamigami:
    def __init__(self, client, BaseFreq = 1.5):
...
class SpringKamigami(Kamigami):

	def getSpringHandles(self):
		self.springHandles = dict()
		error, self.springHandles["left"] = vrep.simxGetObjectHandle(clientID, 'springJointL3#1', vrep.simx_opmode_blocking)
		error, self.springHandles["right"] = vrep.simxGetObjectHandle(clientID, 'springJointR3#1', vrep.simx_opmode_blocking)
	...
	def setSpringConstant(self,leg = "left", k = None, c = None):
		...
		if k != None:
			error_k = vrep.simxSetObjectFloatParameter(clientID, self.springHandles[leg], jointfloatparam_kc_k, k , vrep.simx_opmode_oneshot)
		if c != None:
			error_c = vrep.simxSetObjectFloatParameter(clientID, self.springHandles[leg], jointfloatparam_kc_c, c , vrep.simx_opmode_oneshot)
...
kamigami = SpringKamigami(clientID)
...
with open(filename,'w' , newline='') as csvfile:
    ...
    while loop:     
        ...
        kamigami.controller()
        # if loop == 25:
	       #  kamigami.setSpringConstant(k=20)
